...
Transformation configurations are optional: a configuration is a reusable handle for tuning how an image processes your input, within the scope the image's code already allows. Configurations are JSON documents (a structured, text-based settings format) that define how an input file should be processed, including the input format, metadata extraction rules, and curation logic. Configurations are stored centrally and identified by an integer ID. The same configuration can be reused across multiple input files that share the same structure, and configurations can be created, retrieved, updated, and archived independently of any particular run.
