...
ODM 1.64 introduces **Audit Logs**, providing a structured and searchable record of important user and system actions
across the platform.

* Record key actions related to **users, groups, studies, access, ownership, and file downloads**.
* Capture essential context for each event, including **who performed the action, when it happened, what was affected,
the source of the action, and its result**.
* Track metadata changes when a version is **published** or **restored**, recording which version replaced which.
* Record changes to **study access and ownership** for improved traceability.
* Track **study exports, file downloads, and PDF previews**.
* Access and filter audit events through the dedicated **Audit Log REST API**.
* Restrict access to audit information using the dedicated **View Audit Log** permission.
* Audit access to the Audit Log itself, providing traceability of who reviewed audit information and which filters were used.

Only publishing is logged, so edits sitting in a draft do not appear in the audit log. The entry names the versions
involved, not the values that changed.

For the complete list of supported audit events, event attributes, filtering options, and API usage, see ODM API →
Administer API → Audit Logs section of User Guide.

### Removal of `publish-versions` endpoint

The POST `/api/v1/tasks/publish-versions` endpoint, deprecated in release 1.63, has now been removed.

This endpoint allowed users with **Access all data** permission to publish pending metadata changes across all studies at once.
It was originally introduced to support automatic publication of unpublished data after enabling versioning on an instance.

The endpoint has been removed as part of the permissions and study access control rework.
Publishing metadata changes across multiple studies without approval from the respective study owners is no longer
supported, helping ensure that changes are published within the access and ownership boundaries of each study.

### Improved Access Token support for ODM data import

The ODM data import script can now be run with an **access token** without specifying the optional `template` parameter.

If no template is provided, the script automatically resolves and uses the **default template**, making Access
Token-based imports consistent with other authorization methods.

### Fixed import of multivalue metadata with spaces around `|`

Metadata Editor now correctly imports **Libraries** and **Preparations** files where multivalue fields contain
spaces before or after the pipe (|) separator.

Previously, such files could be imported through the Jobs API but failed when imported through the Metadata Editor UI.

### Fixed missing task logs after publishing a new metadata version

Task logs for imported attachments are now preserved and remain available in **Task Manager** after publishing a
new metadata version of a study.

Previously, the task log could become empty after metadata changes were published.

### Fixed duplicate samples in large sample groups

Publishing a sample group with more than 2,000 samples no longer writes a sample into it twice.

Previously, editing a sample whose row fell beyond the first 2,000 could silently commit it twice, once as your edit
and once as the original it was meant to replace. Once a duplicate had been committed, the group could no longer be
published or discarded, and there was no way to repair it from the interface.

### Security fixes

ODM 1.64 includes several security improvements across **access control, API protection, query handling,
and third-party dependencies**.

The release strengthens authorization checks for data-access endpoints, improves protection against unsafe
query processing, and restricts unintended access to internal application functionality.
It also resolves additional findings identified through static code analysis and vulnerability scanning,
including updates to vulnerable dependencies where patched versions are available.

These changes improve the overall protection of sensitive data and reduce the risk of unauthorized access
or misuse of application interfaces.

### User guide restructure

The ODM user guide has a new structure. Navigation is now purpose-based rather than feature-based, with six
sections: Overview, Explore data, Contribute data, Admin, ODM API, and API Libraries.

!!!note "Links in earlier release notes"

    Pages have moved, so some links in older release notes may not work any more. Use the navigation or search to
    find what you need.
...
Processors Controller jobs and configurations now live in persistent storage, improving traceability and reproducibility.
...
Configurations are now versioned: updating one creates a new version rather than overwriting it. Earlier versions stay retrievable.
...
* **Automatic import after conversion**
    ...
    ![AT Diagram](../assets/user-guide/doc-odm-user-guide/doc-odm-user-guide/images/attachments-transformation-diagram.png)
...
![Add Samples](../assets/user-guide/doc-odm-user-guide/doc-odm-user-guide/images/add-samples-button.png)
...
![Single Sample deletion](../assets/user-guide/doc-odm-user-guide/doc-odm-user-guide/images/sample-object-deletion.png)
...
![Template applied](../assets/user-guide/doc-odm-user-guide/doc-odm-user-guide/images/template-applied-to-studies.png)
...
![PDF Preview](../assets/user-guide/doc-odm-user-guide/doc-odm-user-guide/images/pdf-preview.png)
...
![New token](../assets/user-guide/doc-odm-user-guide/doc-odm-user-guide/images/token.png)
...
![Validity Facet](../assets/user-guide/doc-odm-user-guide/doc-odm-user-guide/images/validity-facet.png)
