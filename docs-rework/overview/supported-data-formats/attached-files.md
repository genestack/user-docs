...
Attachments can be uploaded via the GUI, the `POST /api/v1/jobs/import/file` endpoint, or the `odm-import-data.py` script. Files attached without samples are also supported, useful for storing experiment design documents during early study setup.
