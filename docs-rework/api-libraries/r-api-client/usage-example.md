...
This guide shows how to use the ODM R API Client to authenticate and query samples.

## Requirements

- `odmApi` installed. See [Install](install.md).
- An API token. See [Authentication and tokens](../../odm-api/getting-started/authentication-and-tokens.md).

## Example: search samples and display results

The following script queries samples matching a disease filter and prints the first five results:
...
Replace `ODM_TOKEN` with your API token and `ODM_HOSTNAME` with your ODM instance URL.
