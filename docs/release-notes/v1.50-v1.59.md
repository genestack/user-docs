...
!!!info "Target users"
    ...
    **Advanced** - uusers with access to advanced API functionalities, such as user management and data management operations.
...
Starting with release 1.59, ODM streamlines the process of retrieving gene variants within a specified gene region by introducing the variantFeature parameter. Instead of manually specifying chromosome and interval information from the original VCF file, users can now simply provide a gene name. The system automatically references the associated genome to determine the intervals, reads the file, and returns the relevant data, making the process faster and more user-friendly.
...
* ??? danger "The **variantFeature** parameter has been added to the following endpoints"
...
Starting with this release, ODM enhances the precision of filtering variant data by supporting exact matches in **INFO** fields, improving search specificity. This functionality allows variant data to be retrieved from studies primarily associated with different data types (e.g., transcriptomics), based on exact **INFO** field matches.

This feature is now available across all omics endpoints utilizing this parameter and is fully supported in **vxQuery**, which introduces a slightly different syntax for this operation.
...
* ??? danger "Affected endpoints (for both as-curator and as-users) that include the dedicated parameter INFO"
...
With this release, ODM enhances its GET endpoint capabilities, allowing users to retrieve variant data specifically for samples that are linked to expression data meeting specified filter criteria. This ensures that variant data is only retrieved for samples with matching linked expression data, facilitating integrated analysis between expression and variant datasets.
...
* **Enhancements for VCF File Retrieval and Filtering. Search/Filter for Reference Genome by Variant Group**

To enhance the usability of VCF-related endpoints, the 1.59 release allows users to search and filter variant groups by the associated reference genome. These improvements help ensure the appropriate reference genome is being used for gene variant analysis. The section **referenceGenome** is now displayed in the selected endpoints.
...
* **Enhancements for VCF File Retrieval and Filtering. New Filtering Parameter: referenceGenomeId**

A new query parameter, **referenceGenomeId** has been added to the omics endpoints to enable filtering by Reference Genome ID.
