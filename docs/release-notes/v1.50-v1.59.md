...
user-friendly.
    ...
    ??? success "The `variantFeature` parameter has been added to the following endpoints"
...
in `vxQuery`, which introduces a slightly different syntax for this operation.
    ...
    ??? success "Affected endpoints (for both as-curator and as-users) that include `variantInfo` or `vxQuery` parameter"
        ...
        * GET `/api/v1/as-curator/omics/variant/streamed-data`
        === "Release 1.59 - variantInfo"
            ![INFOupdated.png](1.59/variantInfo.png)
        === "Release 1.59 - vxQuery"
...
ODM is retrieved as a part response. The section `referenceGenome` is now displayed in the selected endpoints.
    ...
    ??? danger "Affected endpoints (for both as-curator and as-users) that include referenceGenome"
        ...
        * GET `/api/v1/as-curator/omics/variant/group`
        ...
        <figcaption>Each response now includes a <strong>referenceGenome</strong> section that provides details about the associated reference genome</figcaption>
...
Reference Genome ID.
    ...
    ??? danger "Affected endpoints"  
        *GET `/api/v1/as-user/omics/variant/group`
        * GET `/api/v1/as-curator/omics/variant/group`
        ![referenceGenomeId](1.59/referenceGenomeId.png)
        <figcaption>The parameter <strong>referenceGenomeId</strong> has been added to selected endpoints, to allow retrieval of variant data filtering for specific Reference Genome ID</figcaption>
...
#### Improved Support for Files (formerly known as Attachments): Retrieve Studies by Files’ Metadata

From this release, attached files metadata are fully indexed and searchable in the Study Browser and APIs. Users can now retrieve studies that contain specific files based on their metadata. This feature streamlines the process of locating relevant studies in large datasets
...
#### Improved Support for Files (formerly known as Attachments): Import files and assign data class upon import via API
...
#### Improved Support for Files (formerly known as Attachments): Import files and assign Data Class via GUI
...
**Improved Support for Files (formerly known as Attachments): Improved Metadata Management for Attached Files**
...
Release 1.59 introduces the ability to delete both reference genomes and imported files via the <code>DELETE /api/v1/manage-data/data</code> endpoint, streamlining data lifecycle management and ensuring project relevance, accuracy, and compliance.
