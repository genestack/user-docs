...
1. **[All Users] Filtering variant data based on genes: variantFeature**
...
2. **[All Users] Filtering by the exact match of key values in INFO fields**
...
3. **[All Users] Adding exQuery to variant streaming endpoint**
...
4. **[All Users] Reference Genome section added to the response**
...
5. **[All Users] New Filtering Parameter: referenceGenomeId**
...
DELETE `/api/v1/manage-data/data` endpoint.
    ...
    ??? abstract "Important considerations to delete Reference Genomes"
        ...
        * Only users with **Manage Data** and **Access all Data** permissions can execute the
          DELETE `/api/v1/manage-data/data` endpoint to delete data, including reference genomes.
        * If the Reference Genome is linked to the VCF group, then the reference genome should be unlinked. As a reference 
          genome is required for filtering by genes, variantFeature parameter is expected to not work for the VCF group 
          without a reference genome.
        * In case the selected object has more than one version, the endpoint deletes all of them.
        * If a Reference Genome is linked to any other data and the Reference Genome is deleted, the endpoint will also 
          delete the linked data associated with it then searching for variant by variantFeature (genes) won't be available 
...
#### Improved Support for Files (formerly known as Attachments)

1. Retrieve Studies by Files’ Metadata
    From this release, attached files metadata are fully indexed and searchable in the Study Browser and APIs.
    Users can now retrieve studies that contain specific files based on their metadata. This feature streamlines
    the process of locating relevant studies in large datasets

    ??? danger "Endpoints for files' metadata retrieval"
...
2. Import files and assign data class upon import via API
    ...
    In addition, curators now have the flexibility to assign a **Data Class** to uploaded files via API endpoints. This improvement enables better organization of files, making it easier to locate and categorize them for downstream analysis.
    ...
    * Request body:
        * The <code>source</code> field is optional. If no source is provided, the default value is used. Allowed values for source are "S3", "HTTP", and "LOCAL".
        * The <code>dataLink</code> field is mandatory and must contain the link to the file to be attached.
        * The <code>dataClass</code> field is mandatory and it contains the data type (e.g. transcriptomics, lipidomics, etc.)
        ...
        * The <code>studyAccession</code> field is mandatory and must specify the ID of the study to which the file will be attached
    * Additional notes:
        * Unlike the GUI, the API does not impose a 5GB size limit for file uploads. This feature is only available to customers with a configured S3 bucket for storage.
...
3. Import files and assign Data Class via GUI

    To complement the API improvements, the graphical user interface (GUI) now supports assigning a **Data Class** when uploading files. This enhancement ensures that non-technical users can achieve the same level of organization without relying on APIs
...
4. Improved Metadata Management for Attached Files
Managing metadata for attached files is now as straightforward as managing metadata for other entities in the system. This includes capabilities for browsing, updating, and searching metadata.

* ??? abstract "Important considerations"
    * Each file is assigned a label (e.g., Bulk transcriptomics) and a unique **accession number**.
    * Linked files (indexed data) display a tick icon and the label **Indexed**.
    * File metadata is indexed and can be searched in the **Study Browser** (and later through APIs)
    * Linked files (e.g., experimental data) will display a **tick icon** and the label **Indexed** to indicate that their content is indexed and searchable. Attached files remain non-indexed but are grouped by their Data Class for clarity.

* ??? "Displayed Metadata in GUI"
...
* **Improved Support for Files (formerly known as Attachments): New APIs for Retrieving File Metadata**
...
* ??? danger "New endpoints to retrieve file's metadata"
...
* **Improved Support for Files (formerly known as Attachments): Download attached files via APIs**
...
* ??? danger "New endpoint to download files"  
    * GET /api/v1/files/{id}/download
    ![Download](1.59/Download.png){width=800}
    <figcaption>The endpoint <code>GET/api/v1/as-user/files/{id}/download</code> allows users to retrieve attached files by indicating the accession number</figcaption>

* **Improved Support for Files (formerly known as Attachments): Improved Export Functionality**

Exporting studies now includes metadata for attached files, enhancing the utility of exported data. The generated file named **[YourStudy]_file_descriptions.txt**, now includes detailed metadata for all attached files.

![Export](1.59/Export.png)
<figcaption>Exported study data now includes metadata for attached files, improving portability and documentation</figcaption>

#### [Admins]

* **Enhanced Data Management via API: Deleting Reference Genomes and Files**

Release 1.59 introduces the ability to delete both reference genomes and imported files via the 
<code>DELETE /api/v1/manage-data/data</code> endpoint, streamlining data lifecycle management and ensuring project 
relevance, accuracy, and compliance.

* ??? abstract "Important considerations to delete Files"
...
* **New “File Manager” app in the Dock Menu**
...
* **Mark initialized/non-initialised files on GUI**

The GUI now visually indicates whether files are indexed or not, providing clarity to users about file status.

* ??? "Important considerations"
    * Indexed files display the label **Indexed Data** and a checkmark icon.
    * Non-indexed files are differentiated, helping users identify files that require additional processing.
    * **Tooltip:** Hovering over the label or icon displays the message: **The entire content of this file is available for search and data exploration via API**.
...
* **Remove outdated endpoints: Eliminating sync POST and DELETE endpoints**
...
* **Performance Optimization for Sample Search API Endpoint**
