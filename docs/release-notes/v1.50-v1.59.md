...
!!! tip "If you've previously uploaded VCF data and data source is valid, it will be automatically migrated to the new output formats."

**[Curators]** Support now includes the ability to upload compressed VCF files up to 1TB. We have also increased
the speed of file uploading and indexing for these large files at the following API endpoints:
...
**[Researchers]** Uploading new VCF files now ensures that the INFO and Genotype fields are correctly split and
parsed based on the data types (String and Float) specified in the VCF file header. This enhancement aids in
constructing more detailed filters for variant identification.

**[Researchers]** Enhanced vxQuery Parameter
...
Affected endpoints (for both as-curator and as-users) that include vxQuery parameter:  
...
**[Researchers]** Changed response format for variant data

List of endpoints (for both as-user and as-curator):  
...
The response format for these endpoints is modified as follows:

**[Researchers]** Retrieving Original VCF Annotation Data
...
**[Researchers]** Data Export in VCF Format

Users can now stream data from a specific VCF file group, applying:
...
Unfiltered requests will export the complete dataset.
...
**[Researchers]** Change to the output format of existing API endpoints

**[All Users]** User Guide Update

A revised user guide is now available, featuring a series of new quick start guide articles.

**[All Users]** Lipidomics Data Class

A new Lipidomics data class is available for selection during the upload of tabular data files.
This new class is integrated within the metadata editor and study browser.

### Fixes

**[Curators]** It's now possible to delete a sub-attribute value, which will remove the entire compound attribute
value.

**[Researchers]** Attribute statistics will now recalculate upon attribute selection.

**[Researchers]** An error message will now display when attempting to export a data file that has been deleted
from the original AWS S3 storage, prompting users to contact the study owner.

**[Curators]** Fixed an issue where the deletion of a related tabular data file could make an entire study unavailable.
