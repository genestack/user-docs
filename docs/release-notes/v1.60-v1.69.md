...
* Bulk propagation: ownership cascades to all Study‑scoped objects (Study, SLP, all associated data objects and their versions).
...
| User with **Manage organisation** *and* **Access all data**  | Study is *not* shared OR user otherwise has access    |   Yes    | Designed for super‑admin use cases.                 |
| User with **Manage organisation** (with access to the Study) | If initiator already has Study access through sharing |   Yes    | Covers org‑level admins who can see the Study.      |
| Any other user                                               | —                                                     |    No    | Not permitted.                                      |

Additional rules:

* Target must be **another active user** (cannot transfer to self; cannot transfer to deactivated accounts).
...
2. Select **Change Owner** (visible if you meet transfer criteria above).
3. Choose the new owner from the active‑user list (searchable dropdown).
...
5. Confirm to execute transfer.
6. Owner field updates immediately.

#### 4. Access Changes After Transfer

* **New owner** receives full access to the Study and all related objects (all versions).
* **Previous owner** loses access *unless* the Study (or specific objects) are explicitly shared back to them.
* Standard sharing rules apply post‑transfer; admins can re‑grant access as needed.

#### 5. Ownership Transfer History in Metadata Versioning

Each transfer generates a new entry in the Study’s metadata Version History:
...
* *Ownership transferred — `User A` transferred ownership to `User B` on 2025‑07‑18 14:32 UTC.* (self‑transfer)
