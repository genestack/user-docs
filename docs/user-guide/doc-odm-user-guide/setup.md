...
4. Prepare the JSON request by specifying the name of the new group. Provide Groups Name in `"displayName"` parameter. 
...
5. From terminal you can use the following curl:
      ...
      ```json title="Curl"
      curl -X 'POST' \
         'https://instance-name.genestack.com/api/v1/scim/Groups' \
         -H 'accept: application/scim+json' \
         -H 'Genestack-API-Token: "your-genestack-token"' \
...
6. The response will confirm that the group has been successfully created, including details such as the group's ID
   and creation date:
      ...
      ```json
      {
       ...
       "id": "autogenerated-genestack:accession",
       "meta": {
         "resourceType": "Group",
