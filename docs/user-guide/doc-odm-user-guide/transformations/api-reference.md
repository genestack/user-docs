...
For how studies are shared, see [Sharing and ownership](../../../access-control/sharing.md); for the Curator group, see [Groups and roles](../../../access-control/groups.md).
...
For `volume_size` sizing guidance (including the per-format recommendations and why 10x H5 files need extra scratch space), see [How to run a transformation](how-to-run-a-transformation.md#step-3-submit-a-dry-run-job). For per-image guidance, see [Available images reference](available-images-reference.md).
