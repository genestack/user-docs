...
!!! note
    This guide applies if your organisation uses Azure Active Directory. For standard token generation, see [Generate an API token](generate-api-token.md).
...
> The token is valid for a limited time (usually 1 hour). You will need to refresh it when expired.
