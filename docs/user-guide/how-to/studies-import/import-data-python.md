# Import data using the Python script

**Role:** Contributor

!!! note
    This guide covers the `import_ODM_data.py` helper script. For direct API calls, see [Import samples (API)](import-samples-api.md) and [Import omics data (API)](import-omics-data-api.md).
...
By default linking is done via the **Sample Source ID** key, so this needs to be consistent in the above files for linking to occur. You can read about linking core data types [here](import-samples-api.md#linking-entities) and more details about signal data linking on [this page](import-omics-data-api.md#linking-to-sampleslibrariespreparations)
